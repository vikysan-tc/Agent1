        ...
        try:
            ...
            if next_at <= now:
                ...
                if state == 'awaiting_upi':
                    ...
                    # Send email notification for UPI request
                    try:
                        from email_sender import send_upi_request_email
                        email_result = send_upi_request_email(
                            customer_email=rec.get('customerEmail'),
                            customer_name='Customer',
                            booking_id=rec.get('bookingId'),
                            ticket_reference=rec.get('ticketReference')
                        )
                        if email_result.get('status') == 'success':
                            print('[EMAIL] UPI request email sent to', rec.get('customerEmail'))
                    except Exception as e:
                        print('[EMAIL] Failed to send UPI request email:', e)
                    ...
                    # Send email notification for refund reinitiation
                    try:
                        from email_sender import send_refund_reinitiated_email
                        ...
                        if email_result.get('status') == 'success':
                            print('[EMAIL] Refund reinitiated email sent to', rec.get('customerEmail'))
                    except Exception as e:
                        print('[EMAIL] Failed to send refund reinitiated email:', e)
